...
export function AuthScreen({
  ...
  const title = useMemo(() => mode === 'login' ? 'Tek panel, doğru rol.' : 'DraBornStyle’a katıl.', [mode]);
  ...
  const submit = () => {
    ...
    const result = onRegister({ fullName, email, phone, password });
    onMessage(result.message, result.ok);
  };
      ...
      >
        <View style={styles.brandRow}>
          ...
          <View>
            ...
            <Text style={styles.version}>v0.1 Final · Rol ve Panel Omurgası</Text>
          </View>
        </View>
        ...
        <View style={styles.hero}>
          ...
          />
          <LinearGradient colors={['rgba(9,11,18,0.05)', 'rgba(9,11,18,0.98)']} style={StyleSheet.absoluteFillObject} />
          <View style={styles.heroCopy}>
            <View style={styles.heroBadge}>
              <Ionicons name="shield-checkmark" size={14} color={colors.green} />
              <Text style={styles.heroBadgeText}>ROL BAZLI GÜVENLİ OMURGA</Text>
            </View>
            <Text style={styles.heroTitle}>{title}</Text>
            <Text style={styles.heroText}>
              Kayıt olan herkes müşteri olur. Usta ve işletme erişimi yalnızca admin onayından sonra açılır.
            </Text>
          </View>
        </View>
          ...
          {mode === 'register' && (
            <View style={styles.roleInfo}>
              ...
              <View style={styles.roleInfoText}>
                ...
                <Text style={styles.roleInfoDetail}>Kayıt sonrası `dkd_user_role_access` içinde müşteri erişimin otomatik açılır.</Text>
              </View>
            </View>
          )}

          <AnimatedPressable style={styles.submitWrap} onPress={submit}>
            <LinearGradient colors={gradients.hero} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.submit}>
          ...
          <View style={styles.demoSection}>
            <View style={styles.sectionHeader}>
              <View>
                ...
                <Text style={styles.sectionText}>Rol yönlendirmelerini tek dokunuşla test et.</Text>
              </View>
              <View style={styles.demoPassword}><Text style={styles.demoPasswordText}>Şifre: 123456</Text></View>
            </View>
              ...
              {demoAccounts.map((account) => (
                <AnimatedPressable
                  ...
                  onPress={() => {
                    ...
                    const result = onLogin(account.email, account.password);
                    onMessage(result.message, result.ok);
                  }}
                >
        ...
        <Text style={styles.footer}>Demo veriler yerel AsyncStorage alanında tutulur · Supabase henüz bağlı değildir.</Text>
      </ScrollView>
    </KeyboardAvoidingView>
  );
}
...
function Field({
  ...
  label,
  ...props
}: React.ComponentProps<typeof TextInput> & { icon: keyof typeof Ionicons.glyphMap; label: string }) {
  return (
    <View>
...
const styles = StyleSheet.create({
  ...
  hero: { minHeight: 230, borderRadius: 28, overflow: 'hidden', borderWidth: 1, borderColor: colors.border },
  heroImage: { width: '100%', height: '100%', position: 'absolute' },
  heroCopy: { flex: 1, justifyContent: 'flex-end', padding: 20 },
  heroBadge: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 7, borderRadius: radii.pill, backgroundColor: 'rgba(8,10,16,0.78)' },
  heroBadgeText: { color: colors.white, fontSize: 9, fontWeight: '900', letterSpacing: 0.8 },
  heroTitle: { color: colors.white, fontSize: 29, lineHeight: 34, fontWeight: '900', marginTop: 13 },
  heroText: { color: colors.textMuted, fontSize: 12, lineHeight: 18, marginTop: 7 },
  ...
  roleInfoDetail: { color: colors.textMuted, fontSize: 10, lineHeight: 15, marginTop: 3 },
  submitWrap: { marginTop: 2 },
  submit: { minHeight: 55, borderRadius: 17, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 16 },
  submitText: { color: colors.white, fontSize: 14, fontWeight: '900' },
  demoSection: { marginTop: 24 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10, marginBottom: 12 },
  sectionTitle: { color: colors.white, fontSize: 18, fontWeight: '900' },
  sectionText: { color: colors.textMuted, fontSize: 10, marginTop: 3 },
  demoPassword: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: radii.pill, backgroundColor: 'rgba(53,225,161,0.1)' },
  demoPasswordText: { color: colors.green, fontSize: 9, fontWeight: '900' },
